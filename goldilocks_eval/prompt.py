...
CATAN_RULES = (
    ...
    "simply be shown the legal placements to choose from. You receive starting "
    "resources from the tiles touching your SECOND opening settlement (one "
    "resource card per adjacent producing tile). After the opening, normal "
    "dice-rolling turns begin.\n\n"

    "ROLLING A 7 AND THE ROBBER: if the two dice add up to 7, no tile produces. "
    "Instead the player who rolled moves the robber onto a DIFFERENT tile (it must "
    "move; it cannot stay where it is). While the robber sits on a tile, that tile "
    "produces nothing for anyone. The roller then steals 1 random resource card "
    "from an opponent who has a settlement or city on the robbed tile (if no such "
    "opponent has any cards, nothing is stolen). Also, whenever a 7 is rolled, any "
    "player holding more than 7 resource cards must discard half of them (rounded "
    "down).\n\n"
